...
static void skippy_hls_demux_pause_tasks (SkippyHLSDemux * demux, gboolean caching);
static gboolean skippy_hls_demux_cache_fragments (SkippyHLSDemux * demux);
static gboolean skippy_hls_demux_schedule (SkippyHLSDemux * demux);
static gboolean skippy_hls_demux_switch_playlist (SkippyHLSDemux * demux);
static gboolean skippy_hls_demux_get_next_fragment (SkippyHLSDemux * demux,
    gboolean caching);
  ...
  switch (event->type) {
    case GST_EVENT_SEEK:
    {
      ...
      while (!g_queue_is_empty (demux->queue)) {
        SkippyFragment *fragment = g_queue_pop_head (demux->queue);
      ...
      skippy_m3u8_client_get_current_position (demux->client, &position);
...
skippy_hls_demux_sink_event (GstPad * pad, GstObject * parent, GstEvent * event)
{
  ...
  switch (event->type) {
    case GST_EVENT_EOS:{
      ...
      playlist = skippy_hls_src_buf_to_utf8_playlist (demux->playlist);
      demux->playlist = NULL;
      if (playlist == NULL) {
...
static void
skippy_hls_demux_pause_tasks (SkippyHLSDemux * demux, gboolean caching)
{
  if (GST_TASK_STATE (demux->updates_task) != GST_TASK_STOPPED) {
    demux->cancelled = TRUE;
    skippy_uri_downloader_cancel (demux->downloader);
...
skippy_hls_demux_stream_loop (SkippyHLSDemux * demux)
{
  ...
  if (G_UNLIKELY (demux->need_cache)) {
    if (!skippy_hls_demux_cache_fragments (demux))
      goto cache_error;
...
end_of_playlist:
  {
    ...
    skippy_hls_demux_pause_tasks (demux, FALSE);
    return;
  }
...
skippy_hls_demux_updates_loop (SkippyHLSDemux * demux)
{
  ...
  while (TRUE) {
    ...
    skippy_hls_demux_schedule (demux);
    ...
    if (skippy_m3u8_client_is_live (demux->client)) {
      if (!skippy_hls_demux_update_playlist (demux, TRUE)) {
        if (demux->cancelled)
          goto quit;
    ...
    if (g_queue_is_empty (demux->queue)) {
      ...
      if (!skippy_hls_demux_get_next_fragment (demux, FALSE)) {
        if (demux->cancelled) {
          goto quit;
        ...
        skippy_hls_demux_switch_playlist (demux);
      }
    }
  }
...
error:
  {
    ...
    skippy_hls_demux_pause_tasks (demux, TRUE);
    g_mutex_unlock (&demux->updates_timed_lock);
  }
}

static gboolean
skippy_hls_demux_cache_fragments (SkippyHLSDemux * demux)
{
  gint i;
  ...
  if (skippy_m3u8_client_has_variant_playlist (demux->client)) {
    ...
    if (!skippy_hls_demux_update_playlist (demux, FALSE)) {
  ...
  for (i = 0; i < demux->fragments_cache; i++) {
    ...
    if (!skippy_hls_demux_get_next_fragment (demux, TRUE)) {
      if (demux->end_of_playlist)
        break;
      if (!demux->cancelled)
...
skippy_hls_demux_update_playlist (SkippyHLSDemux * demux, gboolean update)
{
  ...
  download = skippy_uri_downloader_fetch_uri (demux->downloader, uri);

  if (download == NULL)
    return FALSE;

  buf = skippy_fragment_get_buffer (download);
  playlist = skippy_hls_src_buf_to_utf8_playlist (buf);
  ...
  if (updated && update == FALSE && demux->client->current &&
    ...
    SKIPPY_M3U8_CLIENT_LOCK (demux->client);
    last_sequence =
        SKIPPY_M3U8_MEDIA_FILE (g_list_last (demux->client->current->
            files)->data)->sequence;
...
static gboolean
skippy_hls_demux_schedule (SkippyHLSDemux * demux)
{
  ...
  g_time_val_add (&demux->next_update,
      skippy_m3u8_client_get_target_duration (demux->client)
...
static gboolean
skippy_hls_demux_switch_playlist (SkippyHLSDemux * demux)
{
  GTimeVal now;
  GstClockTime diff;
  gsize size;
    ...
    SkippyFragment * encrypted_fragment, const gchar * key, const guint8 * iv)
{
  ...
  CBC_DECRYPT (&aes_ctx, aes_decrypt, decrypted_info.size, decrypted_info.data, encrypted_info.data);
...
static gboolean
skippy_hls_demux_get_next_fragment (SkippyHLSDemux * demux, gboolean caching)
{
  SkippyFragment *download;
  ...
  download = skippy_uri_downloader_fetch_uri (demux->downloader,
      next_fragment_uri);

  if (download && key)
    download = skippy_hls_demux_decrypt_fragment (demux, download, key, iv);
